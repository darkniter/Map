...
def map_parse(options, fname):
    conf = configobj.ConfigObj(fname, encoding='utf_8')
    result = {}
        ...
        if addresses:
            for unit in table:
                if isinstance(addresses, str):
                    if addresses == unit[3]:
                        _goal.update({'Matched': addresses, row: map[row],
                                        'IP-Old': unit[3], 'Name': row})
                    ...
                    for Ip in addresses:
                        if Ip == unit[3]:
                            _goal.update({'Matched': Ip, row: map[row],
                             'IP-Old': unit[3], 'Name': row})
                        elif Ip == unit[4]:
                            _goal.update({'Matched': Ip, row: map[row],
                             'IP': unit[4], 'Name': row})
...
def main():
    ...
    result_map = map_parse(options, 'smartintel.map')
